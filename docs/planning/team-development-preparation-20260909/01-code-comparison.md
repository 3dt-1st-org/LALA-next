...
조사일: 2026-09-09. 원격 브랜치 SHA와 로컬 Git 객체를 대조했다. 기능 표는 코드·커밋 변경 내용에 대한 판정이며, 실행 성공을 뜻하지 않는다.
...
2026-09-09 GitHub API로 [#187](https://github.com/3dt-1st-org/LALA-next/pull/187)과 [#206](https://github.com/3dt-1st-org/LALA-next/pull/206)을 다시 확인했다. 둘 다 OPEN·Draft다.
...
#206은 main을 직접 기준으로 한 독립 PR이 아니다. 개발 기준 선택 시 이 의존 관계와 포함 변경을 함께 본다. 이번 후속 조회에서 CI·실구동을 재검증하거나 PR을 병합하지 않았다.
...
초기 제안은 후보 코드와 9/8 회의 방향을 출발 자료로 삼는 것이었다. 이후 사용자는 **main·후보를 계속 비교**하기로 선택했다. 후보의 후속 수정과 main에서 선별 통합할 작업을 같은 MVP 기준으로 비교한다.

다만 후보를 main에 병합하거나 팀 개발 브랜치로 확정하는 것은 별개의 결정이다. 우선 화면 범위, 공통 지침, 개발 환경, 지도 기준과 첫 구조 예제를 정리한다. 후보에 있는 기능을 모두 출시하거나, 미룬 기능을 먼저 지우는 작업은 하지 않는다.
