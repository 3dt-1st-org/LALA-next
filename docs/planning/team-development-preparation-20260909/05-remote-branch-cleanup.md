...
상태: 원격 브랜치 34개 삭제 완료. 삭제 전에 복구 태그 34개와 이 문서를 원격에 보존했고, 삭제 후 원격 브랜치가 68개에서 34개로 줄었음을 확인했다.

## 1. 범위와 판정

원격 브랜치 68개와 PR 이력 203개를 확인했다. 삭제 대상은 34개, 보존 대상은 34개다. 아래 항목은 정리 실행 시점의 규칙이다.

- main·dev·당시 준비 브랜치와 당시 열려 있던 PR #187·#206의 head/base는 남긴다.
...
- 로컬 브랜치·worktree·원본 작업 파일은 삭제하지 않는다. 이 원격 정리 작업 안에서는 새 PR 병합·릴리스·운영 배포도 하지 않는다.
...
- 원격 브랜치 68개 중 34개 삭제, 34개 유지.
- 복구 태그 34개가 원격에서 원래 commit SHA를 가리키는 것을 삭제 전에 확인.
- 정확한 head SHA 조건과 atomic push로 삭제. 조사 뒤 head가 달라진 대상은 없었음.
- 삭제 후 대상 브랜치가 모두 사라졌고, 남긴 브랜치는 삭제 직전과 같은 SHA임을 확인.
- 이 원격 정리 시점에는 main·dev·열린 PR #187·#206의 작업선을 유지했고 PR 병합·운영 배포를 수행하지 않았다. 2026-09-10 후속 지시로 #187·#206은 병합했으며 연결된 브랜치는 이번 문서 작업에서 삭제하지 않았다.
- 로컬 remote-tracking 목록은 prune으로 갱신. 로컬 브랜치·15개 worktree·원본 파일은 삭제하지 않음.
- manifest의 branches/sha는 조사 당시 스냅샷이다. 준비 문서 브랜치의 SHA는 결과 기록 커밋에 따라 이후 전진할 수 있음.
