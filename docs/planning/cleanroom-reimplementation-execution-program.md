...
snapshots (e.g. the #59 plan was re-verified in its own worktree on 2026-09-26
at tip `03821a60`, ahead of `main`'s snapshot), so the snapshots are **not**
...
| Plan (draft PR doc) | Domain | Slices owned |
| --- | --- | --- |
| `cleanroom-review-ingestion-reimplementation-plan.md` (#59) | Review/mention ingestion + enrichment | R1 |
| `cleanroom-rag-docent-reimplementation-plan.md` (#58) | RAG → docent / TTS | V1, D1, T1 |
| `cleanroom-day-planner-weather-location-reimplementation-plan.md` (#57) | Day plan + weather/air + location | P1, W1, L1, WA1 |
| `cleanroom-dashboard-map-reimplementation-plan.md` (#55) | Map/dashboard/onboarding | M1, O1, onboarding |
| `cleanroom-restaurant-economy-reimplementation-plan.md` (#56) | Restaurant discovery + local-economy | R2, economy |
...
is another draft PR document (PR #54) and is likewise cited by name, not linked.
...
| --- | --- | --- |
| **CURRENT** | Implemented and live on `main` behind a real DB/API path (or an explicit, honest fallback). | Path + symbol cited; confirmed by direct read of this worktree **or of `main`** (this branch forked `main` at merge-base `2eb9c064`, before the review-governance foundation and the later canonical lanes landed, so `062`–`071` and the later W0/W2–W5 progress are absent from this tree and are cited as "on `main`" with their symbols named). |
...
map client id (Naver Dynamic Map since PR #158), or a placeholder docent is a
**blocked** state, never a passing one.
...
WAVE 0 — shared data contracts / governance ─────────────────────────────────┐
  canonical migration runner ordering · model-role router · review_sources    │
  feature-flag registry · safety-contract test spine · OpenAPI-compat gate    │
  (shipped on main: runner, router, flag registry, safety spine)             │
  └────────────────────────────────────────┬──────────────────────────────────┘
...
WAVE 1 — place data + location/weather     WAVE 2 — review ingestion/         (WAVE 3 reads 1+2)
  ...
  Open-Meteo fallback, is_indoor provenance   receipts/dedupe + DB-backed gate +    summary chunks;
  (reads Wave 0 migration runner)             quarantine (all shipped in 062);      reads Wave 1
                                              072 enrichments replay-audit,
                                              bulk ad classifier, summary-only
                                              RAG hand-off (raw retention           weather/region metadata)
                                              BLOCKED_EXTERNAL)
        │                                      │                                  │
        │                                      ▼                                        │
...
WAVE 4 — 4-slot planner / weather substitutions ◄──────┘ (consumes docent narration + Wave 1 weather/region/hours)
  full 4 slots (shipped), weather_aware_substitute, regenerate,
  intervention history, travel-time
        │
        ▼
WAVE 5 — map/dashboard + restaurant discovery (consumes Wave 1 viewport+region, Wave 2 review quality, Wave 4 plan slots)
  viewport bounds + cluster policy (shipped), cuisine facets,
  Local Restaurant Tour, franchise confidence surfacing, dispersion eval
        │
        ▼
...
| **W2** | Review ingestion/enrichment: **062 governance foundation is the current, complete review-governance baseline** (`ingest.review_sources` provenance registry, `community.ingest_runs` run-accounting extension + `review_source_name` FK, aggregate-only `ingest.review_ingest_receipts` persistent receipt/dedupe, `community.ingest_quarantine`, and the DB-authoritative source gate in the service boundary — no raw-body column, not an immutable ledger; the receipt/dedupe + gate are implemented by that same migration, not held for a separate one), then **072 — `place_enrichments` replay-audit uniqueness (the next review-data migration, TARGET)**, bulk-lane ad classifier, summary-only RAG hand-off, recheck escalation. **Raw review-body retention is BLOCKED_EXTERNAL** (§4.4/DG-11). | W3 grounds docents in **aggregate evidence only** — the `_community_post_chunk` raw-body gap that motivated this wave is closed on `main` (the chunk is now categorical-only; see §4.4), and W2 keeps it closed by making the governed `place_mention` aggregate the only review-derived RAG source. W5's `review_quality_score` and restaurant attributes depend on quarantined-low-confidence discipline from W2. |
...
| **Weather / air snapshot** | `020_travel_domain_tables.sql` (`travel.weather_observations`) + `074` wind_speed/threshold_config (TARGET) | `weather_service.current_weather` (`/api/v1/weather`) | `weather-refresh` job (`weather_observation_refresh`) | `features/weather/widgets/*` | `test_weather_observation_refresh.py`, `weather_map_pill_test.dart` |
| **Review provenance / raw retention** | `030_community_core_tables.sql` + `062` `ingest.review_sources`, `community.ingest_runs` run-accounting ext. (+ `review_source_name` FK), aggregate-only `ingest.review_ingest_receipts` + DB-backed source gate (service-side), `community.ingest_quarantine` (current foundation, all shipped by the one `062` migration; carried by PR #60, merged as `970922c5`, identical on `main`); `072` `travel.place_enrichments` replay-audit uniqueness (TARGET); raw retention **BLOCKED_EXTERNAL** (§4.4/DG-11) | `review_ingest_governance` boundary (no raw-text endpoint — forbidden, §4.4); governed Naver collector staged behind DG-1 (`naver_search_service.py`) | guarded tool `run_review_mention_ingest` (`review_mention_ingest`, `JOB_NAME="review-mention-ingest"`; not yet a `WorkerJobDefinition` in `apps/workers/app/contracts.py`) | (none — aggregate only) | `test_safety_contracts.py` (no-raw-text), `test_review_ingest_governance.py` |
| **Review normalized attributes** | `030` `community.place_mentions_weekly.attributes` + `010` `travel.place_enrichments` mirror (mirror write on `main`; G8 replay-audit unique constraint `072` TARGET) | `review_attribute_batch` (internal; surfaced as aggregate fields on `/places`) | `review-attribute-batch` job (`run_review_attribute_batch`) | (aggregate fields only) | `test_review_attribute_batch.py` |
...
is the current review-governance foundation and is on `main` (carried by PR
...
`062`; the branch head has since advanced to `fc76f3bb`, rebased on `main`
through #214 and re-verified 2026-09-26 — its `062` and its governance tests
are blob-equal to `origin/main`'s, and its governance service differs from
`origin/main`'s only by a docstring clarification of quarantine-count
accounting, with no contract change);
...
separate numbered slice (see below; verified against PR #60's bytes at head
`fc76f3bb` and the #59 plan's re-anchored verification). The **next
review-data migration is therefore the `travel.place_enrichments`
replay-audit uniqueness** (W2-c below). It takes the **first free number
after `062`, which is `072`** (every
number in `063`–`071` is already in use), and the subsequent RAG, planner, and
facet migrations below carry `073`–`075` so every
...
- `062_review_ingestion_governance.sql` (current foundation; carried by PR
  #60, merged to `main` as `970922c5` — at the re-verified branch head
  `fc76f3bb` (2026-09-26) the migration and governance tests are blob-equal to
  `origin/main`'s and the service differs only by a docstring clarification of
  quarantine-count accounting) — additive, re-runnable. Owns
  ...
  received/processed/duplicate/quarantined counters, `failure_category`), the
  aggregate-only `ingest.review_ingest_receipts` persistent receipt/dedupe
  (PK source + external_key + `content_sha256` — no raw text; an exact replay
  is a no-op, a new hash is a content revision), and
  ...
  **DB-authoritative source-gate enforcement and the persistent receipt/dedupe
  are part of this shipped foundation, not a future migration**: the boundary
  loads the source row from `ingest.review_sources`
  (`load_active_review_source`) and aborts the whole batch before any record is
  ...
  holds in both forms; atomic receipt writes with a full-digest
  `aggregate_key`).
- **No separate receipts/gate migration (superseded).** Earlier drafts of this
  program restored the aggregate-only persistent receipt/dedupe + DB-backed
  source gate as a pending `072` migration; that is superseded — they are
  implemented by `062` + the governance service already on `main` (Draft PR
  #60's head carries the same bytes plus test hardening; the #59 plan records
  the same conclusion). No slice may create a migration that re-creates
  `ingest.review_ingest_receipts` or re-implements the source gate.
- `072_place_enrichments_replay_audit.sql` (W2) — **the next review-data
...
- `073_rag_docent_targets.sql` (W3) — `rag.knowledge_chunks.embedding_generation`
...
- `074_planner_data_spine.sql` (W1+W4) — `travel.region_catalog`,
...
- `075_place_facets.sql` (W5) — `travel.places.cuisine_taxonomy`/`cuisine_code`,
  `analytics.recommendation_feedback`, optional `tour_route_cache`.
  These four projections (`072`–`075`) are unique and ordered; any slice whose
...
- **Plan slot schema** = `{period, window, role, place?, weather_fit,
  distance_m, travel_time_min, reason}` in `LalaDailyPlan.slots` (W4). The
  shipped four-period shape on `main` is the fixed order
  `morning/lunch/afternoon/dinner` with honest `unavailable_reason` for
  place-less slots; additive fields stay nullable (§4.3).
- **Viewport/cluster contract** = server returns **places** (≤60, optional
  `bounds`); clustering is the Flutter policy `clusterMapPlacesForMap`
  (`places ≥ 24 || mapLevel ≥ 10`; ≤16 bounded grid cells; selected pin always
  individual) (W5, current truth on `main`).
...
- One ordering authority: `canonical_sql.py` over `sql/canonical/000…075`. A new
...
- No wave redefines a table another wave already shipped. Conflicting additive
  ...
  (e.g., `travel.place_enrichments` uniqueness is owned by W2's `072`).
...
- New endpoints are additive (`/docents/reason`, `/plans/regenerate`,
  `/taxonomies`; the bounds query shipped as additive optional params on
  `/places`, not a new route).
...
- `rag_index._community_post_chunk` was the CONFIRMED gap (it embedded the raw
  `community.posts` body/title into `body_ko`). It is **closed on `main`**: the
  chunk is now categorical-only (keyword/region/provider, no body, title, or
  `post_url`), enforced by
  `test_safety_contracts.py::test_local_signal_and_review_projections_keep_raw_text_out_of_rag_and_docent`.
  This program keeps it closed as an invariant rather than re-fixing it: RAG
  grounds in **aggregate** `place_mention` summaries (counts, sentiment bands,
  attribute scores) only — never a reviewer's verbatim text. The governed
  aggregate → RAG seam is `review_rag_handoff.py`
  (`aggregate_to_place_mention_chunk`), which is aggregate-only by construction
  and guarded by `enforce_no_raw_review_text`.
- No endpoint returns `community.posts.body`. Until the legal/retention/access
  decision (DG-11) clears, raw review text is **not stored, served, logged, or
  embedded** anywhere (BLOCKED_EXTERNAL) — no raw review-body table is created
  by any current slice; should one ever land, it would be forbidden on the serve
  path too. Citations expose source **type/table + record pointer**, not
  embedded content.
- Enforced by `test_safety_contracts.py` (no-raw-text, no-PII-in-aggregates) and
  ...
  which ships the source-gate/receipt enforcement — current).
...
- The legacy reliance on unauthenticated Naver/Daangn scraping is **not** carried
  ...
  (062 + the governance service, current — the DB-backed gate aborts
  disallowed sources),
...
- No scraping/crawling code ships in the repo. The only acquisition lane on
  ...
  only, raw provider text held transiently in memory for hashing/filtering and
  never persisted, opaque sha-based external keys instead of raw URLs, and the
  tool refuses to run until DG-1 registers an active
  `license_class ∈ allowed` source. Unauthenticated scraping and Daangn
...
- **Raw review bodies are not stored at all** until DG-11 (raw-retention
  ...
  PR #60, merged) ships no raw-body column and its receipt/dedupe + DB-backed
...
| **W3-e Audio contract** | Live speech synthesis is shipped on `main` (`/docents/audio` behind `LALA_ENABLE_LIVE_SPEECH` + rate limit, honest 503 when unconfigured, SSML build in `speech_service._build_ssml`, script-cache identity in `docent_service.audio_identity` incl. embedding-generation/retrieval-mode invalidation; response headers `X-LALA-Request-Hash`/`X-LALA-Cache-Key`). Remaining: the `073` `travel.docent_audio_cache` table + `X-LALA-Audio-Cache` hit/miss header + char-limit guard behind `docent_audio_cache`. | Char-limit test; 503-on-failure test; cache-hit test. | Audio cache-hit ≥ target; honest failure state. | Flag `docent_audio_cache`; `LALA_ENABLE_LIVE_SPEECH`. | Medium. G1/G7. |
...
| **W5-f Anonymous feedback + dispersion eval** | `075` `analytics.recommendation_feedback` (session-anonymous, `(place_id,category,day,action_counts)`); offline dispersion eval (Gini/Herfindahl vs card-spend baseline). Privacy: aggregate-only, never joined to identity. | Aggregate-only invariant test; dispersion metric test. | Dispersion eval report artifact (offline). | Flag `RECOMMENDATION_FEEDBACK`. | Medium. Privacy-sensitive. |

### 5.6 Wave 6 — device/runtime E2E + rollout

| Slice | Scope | Tests | Live-data acceptance | Rollback / flag | Risk / dependency |
| --- | --- | --- | --- | --- | --- |
| **W6-a 30–50-place eval set** | Curated bilingual eval set (≤40 places stratified by category/region/language/indoor/context), with grounding anchors and per-case GATE expectations. A 40-place docent QA fixture (`CATEGORY_TARGETS_40`) is already on `main`; this slice completes the bilingual eval set + grounding anchors. No golden prompts; no real review text. | CI runs eval on every pipeline change; recall@3/faithfulness regressions block merge. | Baseline numbers recorded. | None (test fixture). | Medium. Backbone of §9 DoD. |
| **W6-b Real-device captures** | Android physical + iOS/Web parity captures per the visual-acceptance matrix ([`03-visual-acceptance-matrix.md`](./lala-mobile-visual-contract/03-visual-acceptance-matrix.md)): map (live pins), daily plan (KO+EN, weather pair), intervention, regenerate, location-denied, docent/reason, honest states. | Each capture backed by live API (`source`/`grounding_*`; audio-cache header once W3-e lands). | Captures stored under ignored `output/`; no keys/secrets committed. | None. | High. G5. The DoD gate. |
...
| **G-LEGAL** | Licensed/public or processed/de-identified sources only; explicit location consent; **raw review text not stored** (BLOCKED_EXTERNAL). | W2, W6 | `ingest.review_sources` (062) registry + the DB-backed license gate shipped with it (`review_ingest_governance.py`); secret-contract tests; location Opt-in (onboarding S3). |
...
| **Map pin-first then genuine clustering** | Individual category-colored pins first; clusters only under the re-derived policy; selected pin always individual. | W5 | sparse `<24 ⇒ pins`; `≥24 \|\| level≥10 ⇒ bounded cluster grid (≤16 cells)`; selected-individual test (all on `main`). |
...
router (`apps/api/app/services/model_client.py::resolve(role)`, on `main`);
deployment ids/keys are role-based placeholders, never in docs or code. The
canonical role names are the shipped `ModelRole` literals
(`review_bulk`, `review_recheck`, `docent`, `docent_qa`,
`place_enrichment`, `embedding`); earlier plan drafts used
`review_extract`/`recheck_low_conf`/`docent_generate`/`embed`, which
`resolve()` accepts as legacy aliases — new code uses the canonical names.

| Role | Model | Used for | Why this tier |
| --- | --- | --- | --- |
| `review_bulk` (alias `review_extract`) | **gpt-5.4-nano** | High-volume review extraction, normalization, ad classification, keyword/attribute first pass, franchise taxonomy normalization, **indoor/hours classification** (W1-e) | High volume, low marginal value per call → cheapest tier. |
| `review_recheck` (alias `recheck_low_conf`) | **gpt-5.4-mini** | Re-extract/re-classify when nano confidence < threshold; uncertain review signals; ambiguous ad calls | Nuance where nano is uncertain. |
| `docent` (alias `docent_generate`) | **gpt-5.4-mini** | All docent script + reason + planner narration + tour-mode rationale generation | Quality/persona + grounding faithfulness. |
...
| `embedding` (alias `embed`) | **text-embedding-3-small** (1536-d) | Chunk + query embeddings | Matches existing `vector(1536)` schema. |
...
- [ ] **Trustworthy evidence:** every aggregate resolves to `source_run_id` +
      `license_class`; no raw review text in RAG or any API response; no raw
      review-body retention (BLOCKED_EXTERNAL);
      ambiguous/low-confidence signals quarantined, not scored. Verified by
      `test_safety_contracts.py` + `test_review_ingest_governance.py` +
      a DB spot check.
...
| P0-5 | W0-e OpenAPI-compat CI gate | API / platform | OpenAPI export runs in CI on `main`; the compat-baseline wiring remains this slice's deliverable. |
...
| P0-10 | W1-e `is_indoor` provenance (nano + mini recheck) | Enrichment / AI | W0-b (shipped). |

### 10.2 P1 — trustworthy evidence, grounding, and the full plan (Waves 2–4)

| # | Slice | Owner subsystem | Dependency / decision gate |
| --- | --- | --- | --- |
| P1-1 | W2-a review-ingest governance foundation (`062`, current; carried by PR #60, merged) | Ingest / DB | **DG-1** for any live acquisition (**BLOCKED_EXTERNAL**). |
| P1-2 | W2-b aggregate-only persistent receipt/dedupe + DB-backed source gate — **current: shipped inside `062` + the governance service on `main`; no separate migration** | Ingest / DB | **Shipped** (W2-a, current); **DG-1** for any live acquisition (**BLOCKED_EXTERNAL**); **DG-11** only if raw retention ever reopens. |
| P1-3 | W2-c `travel.place_enrichments` replay-audit uniqueness (`072`) — **the next review-data migration** | Ingest / DB | W2-a/W2-b (current); no external gate. |
...
| P1-7 | W3-a real embeddings + reindex generation | RAG / AI | **Largely shipped on `main`** (PR #65: method/generation config, semantic-when-live assertion, stale-only reindex). Remaining: the `073` canonical migration (staged in `sql/operator-pending/`) + backfill run; backfill budget. |
...
| P1-15 | W4-c regenerate + accept | Planner / API | W4-a (shipped); `plan_snapshots` persistence rides `074`. |
| P1-16 | W4-d intervention history + travel-time | Planner / DB | Intervention proposals + Haversine walking time + gated live-routing seam shipped (`travel_time_service.py`); `074` caches + history/diff token remaining. **DG-2** provider choice (**BLOCKED_EXTERNAL**). |

### 10.3 P2 — discovery surfaces, measurement, rollout (Waves 5–6)

| # | Slice | Owner subsystem | Dependency / decision gate |
| --- | --- | --- | --- |
| P2-1 | W5-a viewport-bounds query | Places / API | **Shipped on `main`** behind `PLACES_VIEWPORT_BOUNDS` (bounds params on `/places`). |
| P2-2 | W5-b pin-first + cluster-policy hardening | Flutter / map | **Shipped on `main`** (`≥24 \|\| level≥10`, bounded 4×4/3×3 grid, selected-individual; `map_clustering_test.dart`). |
| P2-3 | W5-c cuisine/meal/diet/indoor facets + `/taxonomies` | Places / Flutter | W1 geo; **DG-?** cuisine taxonomy authority. |
| P2-4 | W5-d Local Restaurant Tour | Flutter / docent | W3-d (citations shipped; reason route pending) + W5-c. |
| P2-5 | W5-e franchise confidence surfacing | Places / Flutter | Underlying fields shipped (`franchise_match_confidence`, `missing_signals` in the score batch); UI surfacing + flag remaining. |
| P2-6 | W5-f anonymous feedback + dispersion eval | Analytics / privacy | Privacy review (aggregate-only). |
| P2-7 | W6-a 30–50-place eval set | QA / eval | **Partially shipped on `main`** (40-place docent QA fixture, `CATEGORY_TARGETS_40`); full 30–50 bilingual eval set with grounding anchors remaining. |
...
| --- | --- | --- | --- |
| **DG-1** | First **licensed review/mention source** + legal sign-off on retention/summarization (e.g. Naver Search API discovery within terms, or an approved export). | No source may be marked `licensed` without it and W6-d ingest rollout cannot call external providers. **BLOCKED_EXTERNAL.** | The `062` foundation (source registry + run accounting + aggregate-only receipts + DB-backed source gate + quarantine) is current (carried by PR #60, merged, identical on `main`); the next review-data migration is `072` `travel.place_enrichments` replay-audit uniqueness (TARGET). A governed Naver Search Open API collector is staged on `main` behind this gate (it refuses to run until the operator registers an active `naver_search` source with an allowed `license_class`; PR #117 lane); until sign-off, no live acquisition runs — `062`/`072` accept already-normalized records only, and the only `community.posts` rows in the tree are the deterministic `sql/dev_reset/` local fixtures (never shipped as live data, §1 hard rule). |
...
| **DG-11** | Raw review-body retention legal/retention/access decision: (a) which sources permit retention, (b) the purge schedule, and (c) the access model. | Any raw review-body retention table and any slice that stores raw review text. **BLOCKED_EXTERNAL.** | No raw review text stored, served, logged, or embedded; `062` ships no raw-body column (current foundation; its receipt/dedupe + DB-backed gate are aggregate-only); the next review migration, `072` (place_enrichments uniqueness), is additive aggregate-only; raw retention stays BLOCKED_EXTERNAL. |
...
> The five domain plans (authored as draft plan documents #55–#59) and the two
> legacy-evidence docs (draft plan document #54) are **not assumed merged**.
...
> snapshots — e.g. the #59 plan's worktree tip `03821a60` — so a `main`
