...
| `image_url` | text | No | TourAPI/KCISA 등 원천 이미지 URL. 없으면 클라이언트가 다른 장소 사진을 재사용하지 않고 중립 썸네일을 표시 |
...
`analytics.place_score_snapshots`의 최신 행을 그대로 사용한다. public demo 모드에서
DB가 없으면 배포 패키지의 `public_mvp_snapshot`을 사용하고 `data_basis=public_mvp_snapshot`으로
표시한다. 이 스냅샷은 공식 API/전처리/개발 시드가 섞인 공개 MVP 캐시이며, 완전한 운영 DB
근거로 과장하지 않는다. 마지막 skeleton fallback만 `data_basis=demo_fallback`으로 표시한
...
| `firstimage`, `detailImage2.originimgurl`, `detailImage2.smallimageurl` | `image_url` | `firstimage`를 우선 보존하고, 비어 있으면 같은 TourAPI `contentid`의 `detailImage2` 공식 이미지를 보강. 앱은 이 값을 우선 사용하고 실패 시 중립 썸네일 표시 |
