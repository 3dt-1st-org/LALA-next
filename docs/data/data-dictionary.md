...
| `source_method` | text | Yes | `azure_openai`, `rule_based_review_terms`, `k_term_api`, `juso_api`, `local_romanization`, `manual` |
...
| `community.place_mentions_weekly` | `review_quality_score` |

`review_quality_score`는 `apps.api.app.tools.run_review_attribute_batch`가
`community.place_mentions_weekly`의 광고 필터 후 집계를 읽어
`attributes.review_quality.score`로 만든다. 유기 리뷰 수가 3건 미만이거나 속성 근거가
부족한 장소는 계속 null로 둔다. 이 값이 있는 장소는
`apps.api.app.tools.run_place_score_batch`가 최신 점수 스냅샷에 반영한다.
