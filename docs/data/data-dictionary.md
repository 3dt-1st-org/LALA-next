...
| `primary_source` | text | Yes | 대표 원천. 예: `tour_api`, `kcisa`, `kopis`, `data_portal`, `local_fixture` |
...
공정거래위원회 가맹사업거래 정보에서 정규화한 브랜드 기준 테이블이다. 추천에서
프랜차이즈 여부를 무조건 배제 조건으로 쓰지 않고, 대형 전국 체인과 지역 소규모
체인을 구분해 소상공인 적합도 보정 신호로 사용한다.

| Column | Type | Required | Description |
|---|---:|---:|---|
| `id` | uuid | Yes | 내부 PK |
| `brand_id` | text | No | 원천 브랜드 식별자 |
| `brand_name_ko` | text | Yes | 원천 브랜드명 |
...
브랜드별 가맹점 목록이 제공하는 지점 단위 참조 테이블이다. 좌표가 있으면 장소명
매칭보다 좌표 근접 매칭을 우선 사용한다.

| Column | Type | Required | Description |
|---|---:|---:|---|
| `id` | uuid | Yes | 내부 PK |
| `franchise_location_id` | text | No | 원천 지점 식별자 |
| `brand_id` | text | No | 브랜드 식별자 |
| `brand_name_ko` | text | Yes | 브랜드명 |
| `normalized_brand_name` | text | Yes | 매칭용 브랜드명 |
| `store_name_ko` | text | No | 원천 지점명 |
| `normalized_store_name` | text | No | 매칭용 지점명 |
| `address_ko` | text | No | 원천 주소 |
| `lat`, `lng` | double precision | No | 좌표 |
| `primary_source` | text | Yes | 기본값 `fair_trade_commission` |
| `source_record_id` | text | No | 원천 레코드 ID |
| `updated_at` | timestamptz | Yes | 마지막 갱신 시각 |
...
| `--apply` | `ALLOW_RAG_INDEX_APPLY=1`과 `--confirm APPLY_RAG_INDEX`가 있을 때만 `rag.knowledge_chunks` upsert 후 `ops.job_runs` 기록 |
...
장소별 프랜차이즈/소상공인 판별 결과다. 이 테이블은 추천 결과를 설명할 때
"왜 로컬 소상공인 적합도가 높거나 낮은지"를 보여주는 근거이며,
`analytics.place_score_snapshots.small_merchant_fit_score`의 입력이다.

| Column | Type | Required | Description |
|---|---:|---:|---|
| `place_id` | text | Yes | `travel.places.place_id` |
| `business_identity_type` | text | Yes | `independent_local`, `local_small_chain`, `franchise_store`, `national_franchise`, `corporate_chain`, `unknown` |
| `is_franchise` | boolean | No | 프랜차이즈 판별값. 불확실하면 null |
| `franchise_brand_name` | text | No | 매칭된 브랜드명 |
| `franchise_match_confidence` | numeric | No | 0~1 매칭 신뢰도 |
| `chain_scale_score` | numeric | No | 0~1 체인 규모 점수 |
| `small_merchant_fit_score` | numeric | No | 추천 계산에 쓰는 소상공인 적합도 |
| `matched_source` | text | No | 원천. 예: `fair_trade_commission` |
| `matched_at` | timestamptz | Yes | 판별 시각 |
| `features` | jsonb | Yes | 정규화 상호명, match type, 거리, 분류 사유 |

`apps.api.app.tools.run_franchise_identity_batch`가 이 테이블을 작성한다. 기본 실행은
plan-only이며 DB에 연결하지 않는다. `--preview`는 `travel.places`,
...
| `small_merchant_fit_score` | numeric | No | 공정위 가맹정보 기반 소상공인/지역 소규모 체인 적합도 |
...
과장하지 않는다. DB와 명시적 스냅샷 fallback이 모두 없을 때는 샘플 장소나 샘플 점수를
만들지 않고 빈 추천 결과를 반환한다.

현재 계산식 가중치:

| Component | Weight | Public-value meaning |
|---|---:|---|
| `local_spending_score` | 0.22 | 내국인 카드 소비/매출 기반 지역경제 활성화 신호 |
| `small_merchant_fit_score` | 0.18 | 공정위 가맹정보 기반 독립 소상공인/지역 소규모 체인 적합도 |
| `demand_dispersion_score` | 0.20 | 과밀 지역 회피와 관광 수요 분산 신호 |
| `culture_relevance_score` | 0.15 | 문체부/TourAPI/KOPIS/KCISA 문화 데이터 연계성 |
| `weather_fit_score` | 0.10 | 날씨/실내외 적합도 |
| `review_quality_score` | 0.10 | 광고 필터 후 리뷰 감성/속성 품질 |
| `accessibility_fit_score` | 0.05 | 동선/실내외/지역 장소 밀도 기반 접근 적합도 |
...
`analytics.place_score_snapshots`에 새 행을 추가한 뒤 `ops.job_runs`에
`place-score-batch` 실행 이력을 남긴다.
...
| `analytics.place_business_identity` | `small_merchant_fit_score` |
...
| `community.place_mentions_weekly` | 충분한 organic review/mention 기반 `review_quality_score` |

`review_quality_score`는 `community.place_mentions_weekly.attributes.review_quality.score`가
있는 충분한 organic evidence 장소에만 채운다. 저증거 장소는 계속 null로 두고
`features.missing_signals`에 `review_attribute_analysis`를 남긴다.

## `ops.job_runs`

수집/전처리/스코어링/RAG 잡 실행 이력이다.
...
| `local_fixture` | 로컬 개발 DB 전용 synthetic fixture |
...
5. `run_place_score_batch`로 추천 계산식을 `analytics.place_score_snapshots`에 버전별로 남기고 `ops.job_runs`에 실행 이력을 기록한다.
6. `run_rag_index`로 정적/동적 맥락을 `rag.knowledge_chunks`에 벡터화해 도슨트와 RAG 검색에 연결하고 `ops.job_runs`에 실행 이력을 기록한다.
...
| `poster` | preview `poster_url` | KOPIS가 `http://www.kopis.or.kr/...` 공식 포스터 URL을 내려주면 preview/export 단계에서 `https`로 정규화 |
...
`culture.events`를 upsert하고 `ingest.source_files`에 실행 해시를 남기며
`ops.job_runs`에 `kopis-ingest` 실행 이력을 기록한다. 수집된 행사는
`run_place_score_batch`에서 지역별 문화 이벤트 신호인 `culture_relevance_score`에 반영된다.
...
| `thumbnail`, `imgUrl`, `image` | preview `thumbnail_url` | 문화포털 공식 이미지가 `http://www.culture.go.kr/...`이면 preview/export 단계에서 `https`로 정규화 |
...
`ingest.source_files`에 실행 해시를 남기며 `ops.job_runs`에
`kcisa-culture-info-ingest` 실행 이력을 기록한다.
...
|---|---|---|
| `기준년월일`, `기준년월`, `ta_ymd`, `std_ym` | `month` | 월 시작일로 정규화 |
| `시군구명`, `시군구코드`, `cty_rgn_no`, `admi_cty_no`, `signgu_cd` | `region_name_ko` | 명칭 우선, 코드만 있으면 경기 시군구 매핑 사용 |
| `카드사 업종분류코드`, `중분류업종코드`, `card_tpbuz_cd`, `mdclass_indutype_cd` | `industry_code` | 원천 코드 보존 |
| `카드사 업종중분류명`, `card_tpbuz_nm_2` | `industry_name_ko` | 상세형 데이터에 있을 때 보존 |
| `성별`, `연령별`, `성연령코드`, `sex`, `age`, `sex_age_cd` | `gender`, `age_group` | `F20`, `M30` 같은 코드는 성별/연령대로 분해하고 상세형 `05` 같은 연령 코드는 `50s`로 정규화 |
| `매출금액`, `sales_amt`, `amt` | `spend_amount` | 월/지역/업종 단위 합산 |
| `매출건수`, `cnt` | `transaction_count` | 값이 있는 원천에서만 합산 |
...
`economy.card_spending_area_monthly`, `economy.card_spending_demographics`에 insert하고
`ops.job_runs`에 `card-spending-file-ingest` 실행 이력을 기록한다. 시군구 코드가 원천
기본 매핑으로 풀리지 않는 경우 `--region-map <csv-or-xlsx>`로 `시군구코드`,
`시군구명` 매핑 파일을 추가할 수 있다.

As of 2026-06-23 UTC, shared dev has both approved Gyeonggi card sources
applied. The aggregate ZIP contributes the broad 31-region baseline through
2025-01, while the detailed `카드소비 데이터_202601-202603.zip` adds 2026-03
coverage for 15 regions, 3,650 area-month-industry rows, and 57,832 demographic
rows. The follow-up `local-value-v2` score batch inserted 2,636 latest score
rows and dynamic RAG regenerated 821 chunks.
