...
| Developer build role | At most `lala-next/kakao-javascript-key`, a domain-restricted client configuration value | `DB_DSN`, OpenAI, Naver, Logto management, API bearer, or public-data keys |
...
`KAKAO_JAVASCRIPT_KEY` is intentionally separate. It is a client build configuration value and must be restricted by the Kakao console to LALA's allowed origins. It may be made available to the dedicated build role only; it does not grant access to server credentials.
...
3. Create a distinct developer build policy allowing only those two actions on `lala-next/kakao-javascript-key`. Do not reuse the EC2 role on laptops.
...
For an iOS, Android, or Web build that renders Kakao map tiles, run the wrapper from the repository root. It supplies only the public API base URL, build SHA, and the domain-restricted Kakao JavaScript key:

```bash
scripts/unix/flutter_with_build_env.sh -- flutter build ios --simulator
```

The wrapper checks a trusted local `.env` first, then the build-only Secrets Manager entry. It does not read or pass API bearer tokens, DB credentials, OpenAI keys, or Logto management secrets. For a non-production endpoint, pass `--api-base-url https://<approved-host>`.
