...
| Item | Current evidence | Next action | Done evidence |
|---|---|---|---|
| Replace contest public access with durable auth | The durable auth lane is code-complete: the Flutter app acquires tokens via the Logto dart SDK (`lib/auth`) and wires the token provider into the main API backend, while the API validates Logto-issued RS256 JWTs and keeps `mode="public"` while guest/contest access remains enabled (merged PRs e9abd91, c701b99, e9a3566). Shared dev still serves the contest window with `LALA_PUBLIC_CONTEST_ACCESS=true`. | Activate externally — no new code: provision the Logto tenant, register the endpoint/app identifiers and API audience in the secret store, build the Flutter app with the Logto dart-defines, disable contest/guest access, then verify device login end-to-end. | The Flutter app authenticates through the Logto SDK, API requests mediate through validated signed tokens with contest/guest access disabled, and a login wall is no longer present for protected surfaces. |
