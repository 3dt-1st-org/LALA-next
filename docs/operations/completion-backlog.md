...
| Skeleton fallback labels | Still exist in code and tests for offline/local safety paths. | They are explicitly marked as fallback/demo data and are not the shared dev happy path. |
...
| Item | Current evidence | Next action | Done evidence |
|---|---|---|---|
| Retire transition bearer token | Shared dev Flutter/API still use a transition bearer token from deployment secrets. | Choose OAuth/Entra token acquisition or a backend-for-frontend proxy for browser clients. | Flutter web no longer ships a static API bearer token; API accepts signed tokens or BFF session credentials. |
