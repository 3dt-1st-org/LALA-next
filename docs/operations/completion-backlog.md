...
| Item | Current evidence | Next action | Done evidence |
|---|---|---|---|
| Replace contest public access with durable auth | API JWT validation and the Flutter Logto SDK foundation are on `main`. Optional onboarding account linking, root-owned session reconciliation, shared token-provider wiring, and public build-config resolution are implemented on the current feature branch pending PR and runtime verification. Guest/contest access remains enabled. | Review and merge the feature PR, verify Web and native hosted login plus `/api/v1/me` reconciliation, then separately approve any guest-access policy change. Do not make authentication a wall for maps, search, planning, weather, or read-only Local Signals. | A selected Logto app signs in through the SDK, protected requests use validated tokens, account sync can recover independently, and guest travel discovery remains usable. |
