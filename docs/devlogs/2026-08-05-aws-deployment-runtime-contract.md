...
`.github/workflows/deploy.yml`이 리터럴 AWS 계정 ID(292216133883), Role ARN, EC2 인스턴스 ID(i-0a15ffbfa381a5843)를 포함하여 민감한 리소스 식별자가 코드베이스에 직접 노출됨.
...
- **systemd 단위 설치 추가**: SSM 명령에 `sudo cp infra/systemd/lala-next-api.service /etc/systemd/system/lala-next.service`, `sudo chmod 644`, `sudo systemctl daemon-reload`, `sudo systemctl is-active lala-next` 추가
...
### 기능 동경 변경
API 기능 동경, RAG 코드, Flutter 코드, 정식 SQL, 무관 문서는 변경하지 않음.
...
4. **헬스체크 실패 처리**: runtime_profile이 `api`가 아니면 배포가 실패하므로 기존 `.env`의 `LALA_RUNTIME_PROFILE` 확인 필요
