...
# SSM Parameter Store name for the build key is operator-supplied only; no default
# is assumed so the script never guesses a secret mapping (CORRECTION_REQUIRED).
KAKAO_KEY_SSM_PARAM="${LALA_KAKAO_KEY_SSM_PARAM:-}"
...
KAKAO_JAVASCRIPT_KEY is resolved least-trusted-source-last: AWS SSM Parameter
Store when --kakao-key-ssm-param (or LALA_KAKAO_KEY_SSM_PARAM) is set, then AWS
...
OpenAI key, or Logto management secret is read.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --kakao-key-ssm-param) KAKAO_KEY_SSM_PARAM="${2:-}"; shift 2 ;;
    --) shift; break ;;
    -h|--help) usage; exit 0 ;;
...
# Build-key resolution order, least-trusted source last:
#   1. AWS SSM Parameter Store -- only when the operator supplies a parameter name
#      (--kakao-key-ssm-param / LALA_KAKAO_KEY_SSM_PARAM). No default name is ever
...
#      KAKAO_JAVASCRIPT_KEY ever reaches this process or the Flutter build.
key=""

if [[ -z "${key//[[:space:]]/}" ]] && [[ -n "$KAKAO_KEY_SSM_PARAM" ]] && command -v aws >/dev/null 2>&1; then
  key="$(aws ssm get-parameter --with-decryption --name "$KAKAO_KEY_SSM_PARAM" --region "$REGION" --query 'Parameter.Value' --output text --no-cli-pager 2>/dev/null || true)"
fi
...
if [[ -z "${key//[[:space:]]/}" ]]; then
  ...
  if [[ -f "$SOURCE_ENV" ]]; then
    ...
    key="$( ( set -a; . "$SOURCE_ENV"; set +a; printf '%s' "${KAKAO_JAVASCRIPT_KEY:-}" ) )"
  fi
fi

if [[ -z "${key//[[:space:]]/}" ]]; then
  echo "KAKAO_JAVASCRIPT_KEY is required from the restricted build secret (SSM/Secrets Manager) or a trusted dotenv file." >&2
  exit 1
fi
