...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan or apply the non-destructive LALA-next canonical SQL baseline."
    )
    parser.add_argument("--json", action="store_true", help="Print machine-readable JSON.")
    parser.add_argument("--apply", action="store_true", help="Apply canonical SQL to DB_DSN.")
    parser.add_argument("--confirm", default="", help=f"Required with --apply: {CONFIRM_TEXT}")
    parser.add_argument("--connect-timeout", type=int, default=5)
    args = parser.parse_args(argv)

    plan = load_canonical_sql_plan()
    if not plan.ok:
    ...
    try:
        result = execute_canonical_sql(dsn=dsn, plan=plan, connect_timeout=args.connect_timeout)
...
def _write(args: argparse.Namespace, payload: dict) -> None:
    ...
    for item in plan.get("files", []):
        print(f"file={item['path']} sha256={item['sha256']} statements={item['statement_count']}")
